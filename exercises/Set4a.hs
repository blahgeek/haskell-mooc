...
distinct s = all (\x -> length (elemIndices x s) == 1) s
...
longest :: (Ord a) => [[a]] -> [a]
longest = let cmp (a:as) (b:bs) = compare (-length as, a) (-length bs, b) in
            minimumBy cmp
...
incrementKey target_k = map (\(k, v) -> if k == target_k then (k, v + 1) else (k, v))
...
winner scores player1 player2 = if score1 >= score2 then player1 else player2
  where score1 = Map.findWithDefault 0 player1 scores
        score2 = Map.findWithDefault 1 player2 scores
...
freqs xs = let increaseMaybe Nothing = Just 1
               increaseMaybe (Just n) = Just $ n + 1 in
  foldr (Map.alter increaseMaybe) Map.empty xs
...
transfer from to amount bank
  | amount > 0 && Map.findWithDefault 0 from bank >= amount && Map.member to bank =
    let alterFrom (Just x) = Just $ x - amount
        alterTo   (Just x) = Just $ x + amount in
      Map.alter alterFrom from $ Map.alter alterTo to bank
  | otherwise = bank
...
maxIndex arr = maximumBy (\idx0 idx1 -> compare (arr ! idx0) (arr ! idx1)) (Data.Array.indices arr)
