...
cull _ Empty = Empty
cull val (Node root left right) = if val == root then Empty else Node root (cull val left) (cull val right)
...
isOrdered (Node root left right) =
  (allValues (\x -> x < root) left) &&
  (allValues (\x -> x > root) right) &&
  isOrdered left &&
  isOrdered right
...
walk (step:step_rem) (Node _ left right) = case step of StepL -> walk step_rem left
                                                        StepR -> walk step_rem right
...
set (step:step_rem) val (Node root left right) =
  case step of StepL -> Node root (set step_rem val left) right
               StepR -> Node root left (set step_rem val right)
...
search needle (Node val left right)
  | needle == val = Just []
  | otherwise = case search needle left of
      Just result -> Just $ StepL : result
      Nothing ->
        case search needle right of
          Just result -> Just $ StepR : result
          Nothing -> Nothing
