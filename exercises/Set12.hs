...
instance Functor List where
  ...
  fmap _ Empty = Empty
  fmap fn (LNode head rest) = LNode (fn head) (fmap fn rest)
...
instance Functor TwoList where
  ...
  fmap fn (TwoNode a b rest) = TwoNode (fn a) (fn b) (fn <$> rest)
...
count needle = foldr (\elem count -> if elem == needle then count + 1 else count) 0
...
inBoth f g = foldMap (\x -> if x `elem` g then [x] else []) f
...
instance Foldable List where
  ...
  foldr fn init (LNode val Empty) = fn val init
  foldr fn init (LNode head rest) = fn head (foldr fn init rest)
...
instance Foldable TwoList where
  ...
  foldr fn init (TwoNode a b TwoEmpty) = fn a (fn b init)
  foldr fn init (TwoNode a b rest) = fn a (fn b (foldr fn init rest))
...
instance Functor Fun where
  fmap f (Fun functor) = Fun $ f . functor
...
instance Functor Tree where
  fmap _ Leaf = Leaf
  fmap fn (Node root left right) = Node (fn root) (fn <$> left) (fn <$> right)

sumTree :: Monoid m => Tree m -> m
sumTree Leaf = mempty
sumTree (Node root left right) = sumTree left <> root <> sumTree right
