...
mapM2 op xs ys = mapM (\(a,b) -> op a b) (zip xs ys)
...
visit maze place = do
  ...
  if place `elem` visited then return ()
    ...
    put $ place:visited
    case lookup place maze of Just neighbors -> forM_ neighbors (visit maze)
                              Nothing -> return ()
...
path maze place1 place2 =
  let reaches = runState (visit maze place1) [] in
    place2 `elem` (snd reaches)
...
instance Functor Result where
  ...
  fmap fn (MkResult res) = MkResult $ fn res
...
instance Functor SL where
  -- implement fmap
  fmap fn (SL op) = SL new_op
    where new_op s = let (res, state, logs) = op s in
            (fn res, state, logs)
...
instance Monad SL where
  -- implement return and >>=
  return val = SL $ \s -> (val, s, [])
  (>>=) (SL op) fn = SL new_op
    where new_op s = let (res, state, logs) = op s
                         SL fn_op = fn res
                         (new_res, new_state, new_logs) = fn_op state in
                       (new_res, new_state, logs ++ new_logs)
...
mkCounter = do
  storage <- newIORef 0
  return (inc storage, get storage)
  where inc storage = modifyIORef storage (1+)
        get storage = readIORef storage
