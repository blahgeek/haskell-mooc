...
member val (Set s) = val `elem` s

-- add a member to a set
add :: (Eq a, Ord a) => a -> Set a -> Set a
add val (Set s) = if member val (Set s) then Set s else Set $ sort (val:s)
...
data State = Start | AddedEggs | AddedFlour | AddedSugar | AddedAll | Mixed | Error | Finished
  deriving (Eq,Show)

step Start AddEggs = AddedEggs
step AddedEggs AddFlour = AddedFlour
step AddedEggs AddSugar = AddedSugar
step AddedFlour AddSugar = AddedAll
step AddedSugar AddFlour = AddedAll
step AddedAll Mix = Mixed
...
reverseNonEmpty (x :| xs) = let fromList l = head l :| tail l in
  fromList $ reverse (x:xs)
...
passwordAllowed passwd (MinimumLength len) = length passwd >= len
passwordAllowed passwd (ContainsSome s) = any (`elem` passwd) s
passwordAllowed passwd (DoesNotContain s) = all (`notElem` passwd) s
passwordAllowed passwd (And a b) = passwordAllowed passwd a && passwordAllowed passwd b
passwordAllowed passwd (Or a b) = passwordAllowed passwd a || passwordAllowed passwd b
...
data Arithmetic = Const Integer | Add Arithmetic Arithmetic | Multiply Arithmetic Arithmetic
  deriving Show
