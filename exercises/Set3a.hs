...
firstHalf s = take (length s - (length s `div` 2)) s
...
whileRight f x = case f x of Left stop_value -> stop_value
                             Right value -> whileRight f value
...
multiApp f gs val = f $ map (\g -> g val) gs
...
interpreter' [] _ _ = []
interpreter' (command:rest) x y
  | command == "up" = interpreter' rest x (y + 1)
  | command == "down" = interpreter' rest x (y - 1)
  | command == "left" = interpreter' rest (x - 1) y
  | command == "right" = interpreter' rest (x + 1) y
  | command == "printX" = show x : interpreter' rest x y
  | command == "printY" = show y : interpreter' rest x y
