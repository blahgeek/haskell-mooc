...
palindromify s
  | isPalindrome s = s
  | otherwise = palindromify trimed_s
  where trimed_s = substring 1 (length s - 1) s
...
greet first last = "Hello, " ++ first ++ last_or_empty ++ "!"
  where last_or_empty = case last of Just x -> " " ++ x
                                     Nothing -> ""
...
addEithers (Left x) (Left _) = Left x
addEithers (Right _) (Left y) = Left y
addEithers (Left x) (Right _) = Left x
